...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_future_question(self):
        # was_published_recently() должен вернуть False,
        # если время в pub_date в будущем.
        time = timezone.now() + datetime.timedelta(days=30)
        future_question = Question(pub_date=time)
        self.assertIs(future_question.was_published_recently(), False)

    def test_was_published_recently_with_old_question(self):
        # was_published_recently() вернет False для вопросов, у которых pub_date
        # более 1 дня назад.
    ...
    def test_was_published_recently_with_recent_question(self):
        # was_published_recently() вернет True для вопросов, у которых pub_date
...
class QuestionIndexViewTests(TestCase):
    def test_no_questions(self):
        ...
        response = self.client.get(reverse('covid:index'))
    ...
    def test_future_question_and_past_question(self):
        ...
        # отображаются только те что в прошлом
        question = create_question(question_text="Вопрос в прошлом", days=-30)
        create_question(question_text="Вопрос в будущем", days=30)
        response = self.client.get(reverse('covid:index'))
        self.assertQuerysetEqual(
...
class QuestionDetailViewTests(TestCase):
    def test_future_question(self):
        # DetailView вопросов с pub_date в будущем
        # вернет ошибку 404 not found.
        future_question = create_question(question_text='Вопрос в будущем', days=5)
        url = reverse('covid:detail', args=(future_question.id,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)

    def test_past_question(self):
        # DetailView вопросов с pub_date в прошлом
        # вернет текст вопроса
        past_question = create_question(question_text='Вопрос в прошлом', days=-5)
        url = reverse('covid:detail', args=(past_question.id,))
        response = self.client.get(url)
