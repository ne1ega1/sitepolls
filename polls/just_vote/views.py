...
class IndexView(ListView):
    ...
    template_name = 'just_vote/index.html'
...
class DetailsView(View):
    ...
    def get(self, request, question_id):
        ...
        if check_user_vote(request, question_id) == True:
            ...
            context = {'question': question,'choices': choices,'error_message': f'Вы уже выбрали ответ "{user_choice}", \
                                                                                  выберите другой вопрос или проголосуйте заново'}

            return TemplateResponse(request, 'just_vote/results.html', context)

        return TemplateResponse(request, 'just_vote/detail.html', {'question': question})


class ResultsView(TemplateView):
    """ Отображение результатов голосования """
    model = Question
    template_name = 'just_vote/results.html'


class RegisterUser(CreateView):
    """ Регистрация """
    form_class = UserCreationForm
    template_name = 'just_vote/register.html'

    def form_valid(self, form):
        user = form.save()
        login(self.request, user)
        return redirect('just_vote:index')


class LoginUser(LoginView):
    """ Авторизация """
    form_class = AuthenticationForm
    template_name = 'just_vote/login.html'
...
class Vote(View):
    ...
    def post(self, request, question_id):
        ...
        if choice_id is None:
            ...
            return TemplateResponse(request, 'just_vote/detail.html', err_context)
